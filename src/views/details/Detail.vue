	...
	<div>
		...
		<div class="content">
			<div class="left">
				...
				<div id="container" class="map"><div id="cesiumContainer"></div></div>
			</div>
			<div class="right">
				<p class="posi">图层</p>
					<el-row class="tac">
						<el-col :span="24">
							<el-menu class="el-menu-vertical-demo" @open="handleOpen" @close="handleClose">
								<el-menu-item index="1"><span slot="title">底图</span><input type="checkbox"></el-menu-item>
								<el-submenu index="2">
									<template slot="title">
										<span>要素图层</span>
									</template>
									<el-menu-item-group>
										<el-menu-item index="2-1">路网<el-checkbox></el-checkbox></el-checkbox></el-menu-item>
										<el-menu-item index="2-2">用地属性<el-checkbox></el-checkbox></el-menu-item>
										<el-menu-item index="2-2">碳排放量<el-checkbox></el-checkbox></el-menu-item>
									</el-menu-item-group>
								</el-submenu>
								<el-menu-item index="3"><span slot="title">色带</span><input type="checkbox"></el-menu-item>
								<el-menu-item index="4"><span slot="title">标注</span><input type="checkbox"></el-menu-item>
								<el-menu-item index="5"><span slot="title">三维表达</span><input type="checkbox"></el-menu-item>
							</el-menu>
						</el-col>
					</el-row>
				<p class="posi">图例</p>
				<div class="bottom" v-if="jenks !== undefined && jenks.length > 1">
					<div>
						<span class="soft legend2"></span>
						<span class="softer1">{{ jenks[0] }} - {{ jenks[1] }}</span>
					</div>
					<div>
						<span class="soft legend2"></span>
						<span class="softer1">{{ jenks[1] }} - {{ jenks[2] }}</span>
					</div>
					<div>
						<span class="soft legend3"></span>
						<span class="softer1">{{ jenks[2] }} - {{ jenks[3] }}</span>
					</div>
					<div>
						<span class="soft legend4"></span>
						<span class="softer1">{{ jenks[3] }} - {{ jenks[4] }}</span>
					</div>
					<div>
						<span class="soft legend5"></span>
						<span class="softer1">{{ jenks[4] }} - {{ jenks[5] }}</span>
					</div>
					<div>
						<span class="soft legend6"></span>
						<span class="softer1">研究范围外</span>
					</div>
				</div>
			</div>
		</div>
	...
	mounted() {
			...
			for (var i = 0; i < entities.length; i++) {
				...
				if (_this.jenks.length > 1) {
					if (_this.jenks[0] < planCe && planCe <= _this.jenks[1]) {
						entity.polygon.material = Cesium.Color.fromCssColorString('#F6FFC1');
					} else if (_this.jenks[1] < planCe && planCe <= _this.jenks[2]) {
						entity.polygon.material = Cesium.Color.fromCssColorString('#BCDAD1');
					} else if (_this.jenks[2] < planCe && planCe <= _this.jenks[3]) {
						entity.polygon.material = Cesium.Color.fromCssColorString('#5CB9C4');
					} else if (_this.jenks[3] < planCe && planCe <= _this.jenks[4]) {
						entity.polygon.material = Cesium.Color.fromCssColorString('#0A759E');
					} else if (_this.jenks[4] < planCe && planCe <= _this.jenks[5]) {
						entity.polygon.material = Cesium.Color.fromCssColorString('#0F4F71');
					} else {
						entity.polygon.material = Cesium.Color.fromCssColorString('#ECECEC');
					}
				} else {
					entity.polygon.material = Cesium.Color.fromCssColorString('#ECECEC');
				}
		...
		handler.setInputAction(function(movement) {
			...
			if (scene.mode !== Cesium.SceneMode.MORPHING) {
				...
				if (scene.pickPositionSupported && Cesium.defined(pickedObject) && pickedObject.id._properties._planCe._value != 0) {
					var cartesian = viewer.scene.pickPosition(movement.endPosition);
					if (Cesium.defined(cartesian)) {
						var cartographic = Cesium.Cartographic.fromCartesian(cartesian);
						labelEntity.position = cartesian;
						labelEntity.label.show = true;
						labelEntity.label.text = pickedObject.id._properties._fid._value + ' : ' + pickedObject.id._properties._planCe._value;

						labelEntity.label.eyeOffset = new Cesium.Cartesian3(0.0, 0.0, -cartographic.height * (scene.mode === Cesium.SceneMode.SCENE2D ? 10 : 10));
